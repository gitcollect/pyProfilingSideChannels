...
class AES(object):
    ...
    def aes_main(self, state, expandedKey, nbrRounds):
        ...
        while i < nbrRounds:
           ...
           #                        self.createRoundKey(expandedKey, 16*i))
            sout_, addin_, state = self.aes_round(state,
                                   self.createRoundKey(expandedKey, 16*i))
            #######
            ##
            if i == 1:
                sout1st = sout_[:]
                addRoundIn2nd = addin_[:]
        ...
        return (addRoundIn2nd, sin1st, sout1st, state)
    ...
    def encrypt(self, iput, key, size):
        ...
        ###################################################################
        ###################################################################
        ain,sin,sout, block = self.aes_main(block, expandedKey, nbrRounds)
        ain_o = [0]*16
        sin_o = [0]*16
        sout_o = [0]*16
        ###################
        ...
        for k in range(4):
            ...
            for l in range(4):
                ...
                ain_o[(k*4)+l] = ain[(k+(l*4))]
                sin_o[(k*4)+l] = sin[(k+(l*4))]
                sout_o[(k*4)+l] = sout[(k+(l*4))]

        return (ain_o,sin_o,sout_o,output)
...
def getMedians(plaintext, key, test = False):
    barr = bytearray(plaintext.decode('hex'))
    p = [int(i) for i in barr]
    if test:
        k = [0] * 32
        k[0] = key
    else:
        barr = bytearray(key.decode('hex'))
        k = [int(i) for i in barr]
    model = AES()
    return  model.encrypt(p,k,32)



if __name__ == "__main__":
    index = u.readIndex(0,1000)
    ain2, sin, sout, c =  getMedians(index[0][1],index[0][0])
    print ain2,sin,sout,c
    print [int(i) for i in bytearray(index[0][2].decode('hex'))]
